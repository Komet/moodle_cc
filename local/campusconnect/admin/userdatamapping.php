...
if ($ecsid && $mid) {
    $params['ecsid'] = $ecsid;
    $params['mid'] = $mid;
    $part = new campusconnect_participantsettings($ecsid, $mid, null, MUST_EXIST);
...
if (!$part) {
    ...
    foreach ($ecslist as $ecsid => $ecsname) {
        ...
        try {
            $communities = campusconnect_participantsettings::load_communities($settings);
    ...
    if (!$allcommunities) {
        ...
        foreach ($allcommunities as $ecsname => $communities) {
            foreach ($communities as $community) {
                /** @var campusconnect_community $community */
                foreach ($community->participants as $participant) {
                    $url = new moodle_url($PAGE->url, array('ecsid' => $participant->get_ecs_id(),
