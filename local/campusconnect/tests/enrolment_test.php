...
class local_campusconnect_enrolment_test extends advanced_testcase {
    ...
    protected function setUp() {
        ...
        foreach ($this->connect as $key => $connect) {
            $communities = campusconnect_participantsettings::load_communities($connect->get_settings());
            $community = reset($communities);
            foreach ($community->participants as $participant) {
                /** campusconnect_participantsettings $participant */
    ...
    protected function clear_ecs_resources() {
        foreach ($this->connect as $connect) {
            ...
            $enrolments = $connect->get_resource_list(campusconnect_event::RES_ENROLMENT, campusconnect_connect::SENT);
            foreach ($enrolments->get_ids() as $eid) {
                $this->connect[1]->delete_resource($eid, campusconnect_event::RES_ENROLMENT);
            }
            // Delete all courselinks.
            $courselinks = $this->connect[2]->get_resource_list(campusconnect_event::RES_COURSELINK, campusconnect_connect::SENT);
            foreach ($courselinks->get_ids() as $cid) {
                $this->connect[2]->delete_resource($cid, campusconnect_event::RES_COURSELINK);
            }
            // Delete all events.
    ...
    public function test_enrolment_status() {
        ...
        $part1 = new campusconnect_participantsettings($this->connect[2]->get_ecs_id(), $this->mid[1]);
        $part1->save_settings(array('export' => true));

        // Set 'unittest1' to import course links from 'unittest2'.
        $part2 = new campusconnect_participantsettings($this->connect[1]->get_ecs_id(), $this->mid[2]);
        ...
        $reslist = $this->connect[1]->get_resource_list(campusconnect_event::RES_ENROLMENT);
        $this->assertEmpty($reslist->get_ids());
        $reslist = $this->connect[1]->get_resource_list(campusconnect_event::RES_COURSELINK);
        $this->assertEmpty($reslist->get_ids());

        // Generate a course and export a course link from 'unittest2' => 'unittest1'.
        $srccourse = $this->getDataGenerator()->create_course(array('fullname' => 'test full name', 'shortname' => 'test short name'));

        $export = new campusconnect_export($srccourse->id);
        $export->set_export($part1->get_identifier(), true);

        campusconnect_export::update_ecs($this->connect[2]); // Export.
        campusconnect_courselink::refresh_from_participant($this->connect[1]->get_ecs_id(), $this->mid[2]); // Import.

        $courselinks = $DB->get_records('local_campusconnect_clink', array('ecsid' => $this->connect[1]->get_ecs_id(),
                                                                           'mid' => $this->mid[2]));
        ...
        $this->assertEquals(campusconnect_enrolment::STATUS_ACTIVE, $enrolstatus->status);

        // Refresh the connection.
        campusconnect_enrolment::update_ecs($this->connect[2]); // Send status 'unittest2' => 'unittest1'.
        ...
        $msgs = $this->connect[1]->get_resource_list(campusconnect_event::RES_ENROLMENT);
        ...
        campusconnect_enrolment::update_ecs($this->connect[2]);
        $msgs = $this->connect[1]->get_resource_list(campusconnect_event::RES_ENROLMENT);
        $this->assertEmpty($msgs->get_ids()); // Check there are now no 'enrolment status' notifications to be received.
        ...
        $this->assertEquals($srccourse->id, $enrolstatus->courseid);
        $this->assertEquals($dstuser->id, $enrolstatus->userid);
        $this->assertEquals(campusconnect_enrolment::STATUS_UNSUBSCRIBED, $enrolstatus->status);
